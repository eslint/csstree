...
describe('parse', () => {
    ...
    describe('basic', () => {
        ...
        describe('@supports', () => {
            const features = [
                ['selector(.example)', 'selector', 'Selector'],
                ['font-tech(color-COLRv1)', 'font-tech', 'Identifier'],
                ['font-format(opentype)', 'font-format', 'Identifier'],
                ['font-format("woff2")', 'font-format', 'String'],
                ['at-rule(@scope)', 'at-rule', 'Raw'],
                ['named-feature(anchor-position-follows-transforms)', 'named-feature', 'Identifier'],
                ['env(safe-area-inset-top)', 'env', 'Identifier']
            ];

            for (const [condition, featureName, valueType] of features) {
                it(condition, () => {
                    const errors = [];
                    ...
                    assert.strictEqual(feature.feature, featureName);
                    assert.strictEqual(feature.value.type, valueType);
                });
            }

            it('should parse nested conditions', () => {
                const errors = [];
                const ast = parse('@supports not (not (transform-origin: 2px)) {}', {
                    onParseError: error => errors.push(error)
                });
