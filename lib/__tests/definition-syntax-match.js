...
function createSyntaxMatchHandler(source) {
    const handler = {
        get match() {
...
function createSyntaxTest(testName, test) {
    (describe[test.test] || describe)(test.name || testName, () => {
        ...
        const matchGraph = createSyntaxMatchHandler(syntax);
        const syntaxes = test.defaultSyntaxes
            ? getDefaultSyntax()
            : { types: {}, properties: {} };

        for (const name of Object.keys(genericSyntaxes)) {
            syntaxes.types[name] = createSyntaxMatchHandler(genericSyntaxes[name]);
        }

        for (const name of Object.keys(test.types || {})) {
            syntaxes.types[name] = createSyntaxMatchHandler(test.types[name]);
        }

        for (const name of Object.keys(test.properties || {})) {
            syntaxes.properties[name] = createSyntaxMatchHandler(test.properties[name]);
        }
