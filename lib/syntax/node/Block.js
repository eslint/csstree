...
function isElementSelectorStart() {
    ...
    // (e.g. `div {`, `div.foo {`, `div > p {`)
    if (nextTokenType !== Colon) {
        return nextTokenType !== Semicolon && nextTokenType !== RightCurlyBracket;
    }

    // An ident followed by a colon is ambiguous: it may be a declaration (`color: red`)
    // or a rule with a pseudo-class or pseudo-element selector (`p:hover {`, `p::before {`).
    // Per CSS Syntax (https://drafts.csswg.org/css-syntax/#consume-block-contents), a declaration
    // is tried first and the content is parsed as a qualified rule only when that fails.
    // A declaration fails when its value contains a top-level {}-block along with other values.
    // The only exception is a custom property, which is always a declaration.
    if (isCustomProperty(this.source, this.tokenStart)) {
        return false;
    }
    ...
    // Scan the tokens following the colon to find out what comes first: a `{` (a nested rule)
    // or a `;` / the end of the enclosing block (a declaration)
    for (let cursor = startToken + this.lookupOffsetNonSC(1) + 1; cursor < this.tokenCount; cursor++) {
        const balanceEnd = this.balance[cursor];

        // Stop on a closing token of a block opened before the ident, i.e. the end of the enclosing block
        if (balanceEnd < startToken) {
            break;
        }

        const tokenType = this.getTokenType(cursor);
        ...
        switch (tokenType) {
            ...
            case Semicolon:
                // Found a semicolon before an opening brace, this is a declaration
                return false;

            case LeftCurlyBracket:
                // Found an opening brace, this is a selector with a block
                // unless the block is the whole value (e.g. `prop: { ... }`)
                return seenValue;

            default:
                seenValue = true;

                // Fast forward to the end of a balanced block (function, parentheses or brackets)
                if (this.isBlockOpenerTokenType(tokenType)) {
                    cursor = balanceEnd;
                }
        }
    }

    // No opening brace found, this is a declaration
    return false;
}
